...
## 0. Use Skills When Relevant

Only invoke a skill when the task clearly matches it — don't scan skills on every task. Common matches for this project:
- **cloudinary-docs / cloudinary-transformations** — image/video uploads, optimization, or transformation URLs.
- **verify / run** — confirming a change works in the running app.
- **code-review / simplify** — reviewing or cleaning up a diff.
- **theme-factory** — applying a cohesive color/font theme.

If the task obviously fits one, use it. Otherwise proceed directly.
